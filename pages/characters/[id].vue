...
const { data } = await useApi<IApiResponse<ICharacter[]>>(
  `/characters/${route.params.id}`
);

const character = computed<ICharacter>(() => data.value?.data.results[0]!);
</script>
  ...
  <div class="py-10">
    <div class="clear-both">
      ...
      <div class="flex flex-col gap-6">
        ...
        <p class="text-xl text-justify">{{ trim(character.description) }}</p>
      </div>
    </div>
  </div>
</template>
